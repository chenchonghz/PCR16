		...
		if(err==OS_ERR_NONE)    {
			if(msg->Src == MSG_USB_START || msg->Src == MSG_USB_READY || msg->Src == MSG_USB_DISCONNECT)	{
				...
				if(msg->Src == MSG_USB_READY||msg->Src == MSG_USB_DISCONNECT)	{
					MountUDISK(msg->Src);
					if(sys.state & SYSSTATE_UDISK_CONNECTED && \
...
//				msg_pkt_udisk.Data = data_buf;
				if(sys.state & SYSSTATE_UDISK_CONNECTED)	{
//					strcpy((char *)data_buf, &Code_Message[2][0]);//显示复制中
					msg_pkt_udisk.Data = (char *)&Code_Message[2][0];
					OSQPost(appdis.MSG_Q,&msg_pkt_udisk);//通知app_display显示
					if(CopyFileToUDISK(LOG_FILE_NAME, LOG_FILE_NAME)==1)	{//copy log file to udisk					
//						strcpy((char *)data_buf, &Code_Message[0][0]);//显示复制成功
						msg_pkt_udisk.Data = (char *)&Code_Message[0][0];
					}
				}else	{
//					strcpy((char *)data_buf, &Code_Message[1][0]);//显示未接u盘
...
//				msg_pkt_udisk.Data = data_buf;
				if(sys.state & SYSSTATE_UDISK_CONNECTED)	{
					char spi_file[FILE_NAME_LEN]/*,perfuse_dat_filename[FILE_NAME_LEN]*/;
					
//					strcpy((char *)data_buf, &Code_Message[2][0]);//显示复制中
					msg_pkt_udisk.Data = (char *)&Code_Message[2][0];
					OSQPost(appdis.MSG_Q,&msg_pkt_udisk);//通知app_display显示
					sprintf(spi_file,"%s/%s", PerfuseDataFileDir, &PerfuseFile.name[PerfuseFile.Cursor][0]);
					if(CopyFileToUDISK(spi_file, &PerfuseFile.name[PerfuseFile.Cursor][0])==1)	{
//						strcpy((char *)data_buf, &Code_Message[0][0]);//显示复制成功
						msg_pkt_udisk.Data = (char *)&Code_Message[0][0];
					}
				}else	{
