...
static void upload_start(transmit_ctrl_t *ptfile)
{
	...
	if(ptfile->fp == NULL)	{//文件不存在 回复文件大小0
		...
		temp = ptfile->size_total/TransmitMaxBufSize;
		if((ptfile->size_total%TransmitMaxBufSize)>0)	temp += 1;//文件分几个序列传输
		memcpy(ptfile->buf_addr+4, (u8 *)temp, 2);
    ...
    switch (pdata[0])
    {
        case TransmitCmd_Start://传输启动
            if (ptfile->state == DEF_Idle) {
				strcpy(ptfile->filepath, (char *)pdata);//上传文件路径
